...
var MediaViews = function (_mix$with) {
  ...
  _createClass(MediaViews, [{
    ...
    value: function popParams() {
      ...
      var params = this.validateParams(this.parseQueryString('media'));
    ...
    value: function pushParams() {
      var media = $(this.config.select2Input).select2('val') || [];

      if (window.history && window.history.replaceState) {
        window.history.replaceState({}, document.title, '?' + $.param(this.getParams()) + '&media=' + media.join('|'));
      }
    ...
    key: 'getMediaViews',
    value: function getMediaViews(filenames) {
      var _this2 = this;
      ...
      var makeRequest = function makeRequest(entity, index) {
        var startDate = _this2.daterangepicker.startDate.startOf('day'),
            endDate = _this2.daterangepicker.endDate.startOf('day'),
            url = 'https://tools.wmflabs.org/mediaplaycounts/api/1/FilePlaycount/date_range/' + (entity.score() + '/' + startDate.format('YYYYMMDD') + '/' + endDate.format('YYYYMMDD')),
            promise = $.ajax({ url: url, dataType: 'jsonp' });
      ...
      filenames.forEach(function (entity, index) {
    ...
    value: function setupDataSourceSelector() {
      ...
      $(this.config.dataSourceSelector).on('change', function (e) {
        var value = $(_this4.config.platformSelector).val() || '',
        ...
        if (_this4.isPageviews()) {
          ...
          $(_this4.config.agentSelector).prop('disabled', false);
        } else {
          $('.platform-select--mobile-web, .platform-select--mobile-app').hide();
          $('.platform-select--mobile').show();
          $(_this4.config.agentSelector).val('user').prop('disabled', true);
        }
        ...
        if (wasMobileValue && _this4.isUniqueDevices()) {
          $(_this4.config.platformSelector).val('mobile-site'); // chart will automatically re-render
        } else if (wasMobileValue && _this4.isPageviews()) {
            $(_this4.config.platformSelector).val('mobile-web');
          }
    ...
    value: function processInput(force) {
      ...
      var media = $(config.select2Input).select2('val') || [];
      ...
      this.getMediaViews(media).done(function (xhrData) {
    ...
    value: function validateProjects() {
      ...
      return projects.filter(function (project) {
        if (siteDomains.includes(project)) {
          ...
          _this7.writeMessage($.i18n('invalid-project', '<a href=\'//' + project.escape() + '\'>' + project.escape() + '</a>'));
          return false;
        }
      });
    }
    ...
    _createClass(_class, [{
      ...
      value: function buildChartData(datasets, labels) {
        ...
        return datasets.map(function (dataset, index) {
          ...
          dataset.forEach(function (elem) {
            var value = _this4.isPageviews() ? elem.views : elem.devices;
            var date = moment(elem.timestamp, _this4.config.timestampFormat).format('YYYY-MM-DD');
      ...
      value: function setColorsAndLogValues(outputData) {
        ...
        return outputData.map(function (dataset, index) {
          ...
          var startDate = moment(_this5.daterangepicker.startDate);
          var endDate = moment(_this5.daterangepicker.endDate);
          ...
          for (var date = startDate; date <= endDate; date.add(1, 'd')) {
            if (!dataset.data[counter]) {
              var edgeCase = date.isSame(_this5.config.maxDate) || date.isSame(moment(_this5.config.maxDate).subtract(1, 'days'));
