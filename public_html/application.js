...
var PageViews = function (_mix$with) {
  ...
  _createClass(PageViews, [{
    ...
    value: function showSinglePageLegend() {
      ...
      var topviewsMonth = moment().subtract(1, 'month').subtract(2, 'days');
      var topviewsDate = this.daterangepicker.endDate.format('YYYY') + '/' + topviewsMonth.format('MM') + '/all-days';
      ...
      $.ajax({
        ...
        $('.table-view').hide();
        $('.single-page-stats').html('\n        ' + _this7.getPageLink(page.label) + '\n        &middot;\n        <span class=\'text-muted\'>\n          ' + $(_this7.config.dateRangeSelector).val() + '\n        </span>\n        &middot;\n        ' + $.i18n('num-pageviews', _this7.formatNumber(page.sum)) + '\n        <span class=\'hidden-lg\'>\n          (' + _this7.formatNumber(page.average) + '/' + $.i18n('day') + ')\n        </span>\n      ');
        $('.single-page-legend').html(_this7.config.templates.chartLegend(_this7));
      });
    }
    ...
    value: function updateTable() {
      ...
      if (!$.isNumeric(this.outputData[0].num_edits)) {
        $('.legend-block--revisions .legend-block--body').html("<span class='text-muted'>Data unavailable</span>" // FIXME: i18n
        );
      }
      ...
      var totals = {
        ...
        num_edits: datasets.reduce(function (a, b) {
          return a + b.num_edits;
        }, 0),
        num_users: datasets.reduce(function (a, b) {
      ...
      this.getPageInfo(pages).done(function (data) {
        ...
        for (var page in data) {
          if (data[page].missing) {
            _this9.writeMessage(_this9.getPageLink(page) + ': ' + $.i18n('api-error-no-data'));
            delete data[page];
          }
        }
        ...
        _this9.getEditData(Object.keys(data)).done(function (editData) {
          for (var _page in editData.pages) {
            ...
            var protection = (_this9.entityInfo[_page].protection || []).find(function (prot) {
              return prot.type === 'edit';
            });
            pageData.protection = protection ? protection.level : $.i18n('none').toLowerCase();

            Object.assign(_this9.entityInfo[_page], editData.pages[_page]);
          }
          dfd.resolve(_this9.entityInfo);
        }).fail(function () {
          dfd.resolve(_this9.entityInfo); // treat as if successful, simply won't show the data
        });
      }).fail(function () {
    ...
    value: function massviewsRedirectWithPagePile(pages) {
      ...
      $.ajax({
        ...
        _this10.toastError($.i18n('auto-pagepile-error', 'PagePile', 10));
